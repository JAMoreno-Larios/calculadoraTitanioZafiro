...
{
    parsePtr=Par;
    cajaTexto = new salidaTexto(this);
    ui->setupUi(this);
}
...
//    worker *trabajo = new worker(parsePtr,ui->avanceSpots,ui->avanceTermico,ui->avanceNoLinealEM1,ui->avanceNoLinealEM2,ui->salidaTexto);
//    trabajo->moveToThread(hilo);
//    connect(trabajo, SIGNAL(error(QString)),this,SLOT(errorString(QString)));
//    connect(hilo,SIGNAL(started()),trabajo,SLOT(process()));
//    connect(trabajo,SIGNAL(finished()),hilo,SLOT(quit()));
//    connect(trabajo,SIGNAL(finished()),trabajo,SLOT(deleteLater()));
...
void interfaz1::setDefaults()
{
    ...
    parsePtr->mapaChar["RutaEM2ML"]=this->RutaEM2ML=(char*)malloc(caracteres+1);
    caracteres=snprintf(NULL,0,".");
    this->RutaIteracionesEM1=(char*)malloc(caracteres+1);
    this->RutaIteracionesEM2=(char*)malloc(caracteres+1);
    ...
    sprintf(this->RutaIteracionesEM1,".");
    sprintf(this->RutaIteracionesEM2,".");
...
void interfaz1::on_mapaCW_toggled(bool checked)
{
    if(checked)
    {
        ui->epsilon1_min->setEnabled(true);
...
void interfaz1::on_actionVer_diagrama_triggered()
{
    // Crea diálogo no modal y muestra la imagen
    QDialog *dialogo = new QDialog(this);
    dialogo->setWindowTitle("Diagrama de la cavidad");
    QImage cavidad;
    cavidad.load(":/recursos/cavidad.png");
    int alto=cavidad.height();
    int ancho=cavidad.width();
    dialogo->setFixedHeight(alto);
    dialogo->setFixedWidth(ancho);
    QLabel *fondo=new QLabel(dialogo);
    fondo->setPixmap(QPixmap::fromImage(cavidad));
    fondo->show();
...
void interfaz1::on_initCalculloCW_clicked()
{
    if(parsePtr->archivoVacio())
        parsePtr->escribeDatos(parsePtr->mapaChar,parsePtr->mapaBool,parsePtr->mapaEnteros,parsePtr->mapaDobles);
    parsePtr->cargaVariables();
    ...
    anguloLineal(conjugado_cortoLocal,conjugado_largoLocal,L,n,L1,L2,f1,f2,angulos);

    delta1=distanciaCristal(conjugado_cortoLocal,f1,L1,L,n,angulos[0]);
    delta2=distanciaCristal(conjugado_largoLocal,f2,L2,L,n,angulos[1]);
    QString cwSimple;
    cwSimple.append(QString("Cálculo en onda continua\n"));
    cwSimple.append(QString("Ángulos:\n θ1 = %1 [°]\tθ2 = %2 [°]\n\n").arg((double)angulos[0]*180.0/M_PI).arg((double)angulos[1]*180.0/M_PI));
    cwSimple.append(QString("Distancia espejo cóncavo  a cristal:\n δ1 = %1 [mm]\tδ2 = %2 [mm]\n\n").arg((double)delta1*1e3).arg((double)delta2*1e3));
    cajaTexto->escribeLineaTexto(cwSimple);

    if(ui->mapaCW->isChecked())
    {
        epsilon1Min=(long double)ui->epsilon1_min->value()*1e-3;
        epsilon1Max=(long double)ui->epsilon1_max->value()*1e-3;
        epsilon2Min=(long double)ui->epsilon2_min->value()*1e-3;
        ...
        double spotTanEM1CW, spotSagEM1CW, spotTanEM2CW, spotSagEM2CW;
        ...
        long double angulos[2];
        anguloLineal(conjugado_cortoLocal,conjugado_largoLocal,L,n,L1,L2,f1,f2,angulos);
        double deltaFin1EM1=distanciaCristal(conjugado_cortoLocal,f1,L1,L,n,angulos[0])+creall(obtieneElemento(epsilon1,1,e1PosEM1CW));
        double deltaFin2EM1=distanciaCristal(conjugado_largoLocal,f2,L2,L,n,angulos[1])+creall(obtieneElemento(epsilon2,1,e2PosEM1CW));

        double deltaFin1EM2=distanciaCristal(conjugado_cortoLocal,f1,L1,L,n,angulos[0])+creall(obtieneElemento(epsilon1,1,e1PosEM2CW));
        double deltaFin2EM2=distanciaCristal(conjugado_largoLocal,f2,L2,L,n,angulos[1])+creall(obtieneElemento(epsilon2,1,e2PosEM2CW));

        QString cwDeltasVariables;
        cwDeltasVariables.append(QString("Para cavidad vista desde EM1, configuración menos astigmática:\n"));
        cwDeltasVariables.append(QString("Distancia entre espejo cóncavo y cristal: δ1 = %1 [mm]\tδ2 = %2[mm]\n").arg(deltaFin1EM1*1e3,deltaFin2EM1*1e3));
        cwDeltasVariables.append(QString("Radio de haz tangencial ωt en EM1: ωt = %1 [mm]\n").arg(spotTanEM1CW*1e3));
        cwDeltasVariables.append(QString("Radio de haz sagitall ωs en EM1: ωs = %1 [mm]\n").arg(spotSagEM1CW*1e3));
        cwDeltasVariables.append(QString("\n"));
        cwDeltasVariables.append(QString("Para cavidad vista desde EM2, configuración menos astigmática:\n"));
        cwDeltasVariables.append(QString("Distancia entre espejo cóncavo y cristal: δ1 = %1 [mm]\tδ2 = %2 [mm]\n").arg(deltaFin1EM2*1e3,deltaFin2EM2*1e3));
        cwDeltasVariables.append(QString("Radio de haz tangencial ωt en EM2: ωt = %1 [mm]\n").arg(spotTanEM2CW*1e3));
        cwDeltasVariables.append(QString("Radio de haz sagitall ωs en EM2: ωs = %1 [mm]\n").arg(spotSagEM2CW*1e3));
        cajaTexto->escribeLineaTexto(cwDeltasVariables);
